...
void food(ArrayList<Food> list_f) {
  ...
  for(Food f : list_f) {
    ...
    // println(f.size, f.size_ref, float(f.size) / float(f.size_ref)) ;
    // println(f.colour, g.colorModeA, alpha) ;
...
// local method
void search_food_target(Herbivore h, ArrayList<Food> list_food_target) {
  if(!h.calm) {
    for(Food target : list_food_target) {
      h.watch(target) ;
      h.pick(target) ;
      if(h.picking()) break ;
    }
