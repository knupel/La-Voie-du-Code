...
//raw compare
float longest_word( String[] listWordsToSort) {
  float size_word = 0 ;
  for ( int i = 0 ; i < listWordsToSort.length ; i++) {
    if (listWordsToSort[i].length() > size_word) size_word = listWordsToSort[i].length() ;
  }
  return size_word ;
}
//with starting and end keypoint in the String must be sort
float  longest_word( String[] listWordsToSort, int start, int finish ) {
  float  size_word = 0 ;
...
float longest_word_in_pixel(String[] listWordsToSort, int size_font) {
  float size_word = 0 ;
  for ( int i = 0 ; i < listWordsToSort.length ; i++) {
    if (width_String(listWordsToSort[i], size_font) > size_word) size_word = width_String(listWordsToSort[i],size_font) ;
  }
  return size_word ;
}

// with the same size_text for each line, choice the which line you check
float longest_word_in_pixel( String[] listWordsToSort, int size_font, int start, int finish ) {
  float size_word = 0 ;
  for ( int i = start ; i <= finish ; i++) {
    if (width_String(listWordsToSort[i], size_font) > size_word) size_word = width_String(listWordsToSort[i],size_font) ;
  }
...
float longest_word_in_pixel( String[] listWordsToSort, int [] size_font) {
  float size_word = 0 ;
  for ( int i = 0 ; i < listWordsToSort.length ; i++) {
    if (width_String(listWordsToSort[i], size_font[i]) > size_word) size_word = width_String(listWordsToSort[i],size_font[i]) ;
  }
  return size_word ;
}
...
float width_String(String target, int size_font) {
  PFont font = createFont("defaultFont", size_font) ;
  return width_String(font, target, size_font) ;
}


float width_String(PFont font, String target, int size_font) {
  float string_width = 0 ;
  String [] split = target.split("") ;
  for(int i = 0 ; i < split.length ; i++) {
    char c = split[i].charAt(0) ;
    string_width += width_char(font, c) ;
  }
  return string_width *size_font ;
}


float width_char(PFont font, char target) {
  return font.width(target) ;
}
